    ...
    <div class="content container-fluid">
        ...
        <div class="card">
            ...
            <div class="card-header d-block p-3">
                <span
                    class="badge badge-soft-primary white--space m-1 text-left">{{ translate('messages.map_api_hint') }}</span>
                <span
                    class="badge badge-soft-primary white--space m-1 text-left">{{ translate('messages.map_api_hint_2') }}</span>
            </div>
            <div class="card-body">
                <form action="{{ env('APP_MODE') != 'demo' ? route('admin.business-settings.config-update') : 'javascript:' }}"
                    method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row gy-3">
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                <label class="input-label">{{ translate('messages.map_api_key') }}
                                    ({{ translate('messages.client') }})</label>
                                <input type="text"
                                    placeholder="{{ translate('messages.map_api_key') }} ({{ translate('messages.client') }})"
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                <label class="input-label">{{ translate('messages.map_api_key') }}
                                    ({{ translate('messages.server') }})</label>
                                <input type="text"
                                    placeholder="{{ translate('messages.map_api_key') }} ({{ translate('messages.server') }})"
