            ...
            <div class="card mb-3 p-5">
                <div class="card-body">
                    <p class=" card-title text-center">
                        <code>{{ translate('You have already added maximum numbers of banner image') }}</code></p>
                </div>
            </div>
        @else
            <div class="card mb-3">
                <div class="card-body">
                    <form action="{{ route('admin.business-settings.landing-page-settings', 'promotion-banner') }}"
                        method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label class="input-label" for="title">{{ translate('messages.title') }}</label>
                            <input type="text" placeholder="{{ translate('messages.title') }}" id="title"
                                name="title" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label class="input-label" for="sub_title">{{ translate('messages.sub') }}
                                {{ translate('messages.title') }}</label>
                            <input type="text" placeholder="{{ translate('messages.sub_title') }}" id="sub_title"
                                name="sub_title" class="form-control" required>
                        </div>

                        <div class="form-group">
                            <label class="input-label">{{ translate('messages.banner') }} <small class="text-danger">* (
                    ...
                    <table class="review--table table table-borderless table-thead-bordered table-align-middle card-table">
                        <thead class="thead-light">
                            <tr>
                                ...
                                <th scope="col" class="w--2 border-0 text-center">
                                    {{ translate('messages.banner_image') }}</th>
                            ...
                            @if ($promotion_banner)
                                @foreach ($promotion_banner as $key => $promotion)
                                    <tr>
                                        ...
                                        <td>
                                            <img class="avatar avatar-lg mr-3 w-100"
                                                src="{{ asset('assets/landing/image') }}/{{ $promotion['img'] }}"
                                        ...
                                        <td>
                                            <div class="btn--container justify-content-center">
                                                <a class="btn action-btn btn--danger btn-outline-danger" href="javascript:"
                                                    onclick="form_alert('promotion-{{ $key }}','{{ translate('messages.Want_to_delete_this_item') }}')"
                                                    title="{{ translate('messages.delete') }}"><i
                                                        class="tio-delete-outlined"></i>
                                                </a>
                                            </div>
                                            <form
                                                action="{{ route('admin.business-settings.landing-page-settings-delete', ['tab' => 'promotion_banner', 'key' => $key]) }}"
