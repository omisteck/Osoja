        ...
        <ul class="nav nav-tabs page-header-tabs mb-2">
            <li class="nav-item">
                <a href="{{ route('admin.transactions.report.store-summary-report') }}"
                    class="nav-link active">{{ translate('Summary Report') }}</a>
            </li>
            <li class="nav-item">
        ...
        <div class="card border-0 mb-3">
            <div class="card-body">
                <div class="statistics-btn-grp">
                    <label>
                        <input type="radio" name="filter" value="all_time"
                            {{ isset($filter) && $filter == 'all_time' ? 'checked' : '' }}
                            onchange="set_time_filter('{{ url()->full() }}',this.value)" hidden>
                        <span>{{ translate('All Time') }}</span>
                    </label>
                    <label>
                        <input type="radio" name="filter" value="this_year"
                            {{ isset($filter) && $filter == 'this_year' ? 'checked' : '' }}
                            onchange="set_time_filter('{{ url()->full() }}',this.value)" hidden>
                        <span>{{ translate('This Year') }}</span>
                    </label>
                    <label>
                        <input type="radio" name="filter" value="previous_year"
                            {{ isset($filter) && $filter == 'previous_year' ? 'checked' : '' }}
                            onchange="set_time_filter('{{ url()->full() }}',this.value)" hidden>
                        <span>{{ translate('Previous Year') }}</span>
                    </label>
                    <label>
                        <input type="radio" name="filter" value="this_month"
                            {{ isset($filter) && $filter == 'this_month' ? 'checked' : '' }}
                            onchange="set_time_filter('{{ url()->full() }}',this.value)" hidden>
                        <span>{{ translate('This Month') }}</span>
                    </label>
                    <label>
                        <input type="radio" name="filter" value="this_week"
                            {{ isset($filter) && $filter == 'this_week' ? 'checked' : '' }}
                            onchange="set_time_filter('{{ url()->full() }}',this.value)" hidden>
                    ...
                    <div class="hs-unfold mr-2">
                        ...
                        <div id="usersExportDropdown"
                            ...
                            <a id="export-excel" class="dropdown-item"
                                href="{{ route('admin.transactions.report.store-summary-report-export', ['type' => 'excel', request()->getQueryString()]) }}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                                    src="{{ asset('assets/admin') }}/svg/components/excel.svg" alt="Image Description">
                                {{ translate('messages.excel') }}
                            </a>
                            <a id="export-csv" class="dropdown-item"
                                href="{{ route('admin.transactions.report.store-summary-report-export', ['type' => 'csv', request()->getQueryString()]) }}">
                                <img class="avatar avatar-xss avatar-4by3 mr-2"
                        ...
                        <thead class="thead-light white--space-false">
                            <tr>
                                ...
                                <th class="border-top border-bottom text-capitalize">
                                    {{ translate('Total Delivered Order') }}</th>
                                <th class="border-top border-bottom text-capitalize text-center">
                                    {{ translate('Total Amount') }}</th>
                                <th class="border-top border-bottom text-capitalize text-center">
                                    {{ translate('Completion Rate') }}</th>
                                <th class="border-top border-bottom text-capitalize text-center">
                                    {{ translate('Ongoing Rate') }}</th>
                                <th class="border-top border-bottom text-capitalize text-center">
                                    {{ translate('Cancelation Rate') }}</th>
                                <th class="border-top border-bottom text-capitalize text-center">
                                    {{ translate('Refund Request') }}</th>
                        ...
                        <tbody id="set-rows">
                            @foreach ($stores as $k => $store)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a
                                            href="{{ route('admin.store.view', [$store->id, 'module_id' => $store->module_id]) }}">{{ $store->name }}</a>
                                    </td>
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a href="{{ route('admin.store.view', [$store->id, 'module_id' => $store->module_id]) }}"
...
@push('script_2')
    ...
    <script>
        var options = {
            series: [
                {{ count($order_payment_methods) > 0 ? (isset($order_payment_methods[0]) ? $order_payment_methods[0]->order_count : 0) : 0 }},
                {{ count($order_payment_methods) > 0 ? (isset($order_payment_methods[1]) ? $order_payment_methods[1]->order_count : 0) : 0 }},
                {{ count($order_payment_methods) > 0 ? (isset($order_payment_methods[2]) ? $order_payment_methods[2]->order_count : 0) : 0 }}
            ],
            chart: {
                width: 320,
                type: 'donut',
            ...
            labels: [
                'Cash Payments ({{ count($order_payment_methods) > 0 ? (isset($order_payment_methods[0]) ? $order_payment_methods[0]->total_order_amount : 0) : 0 }})',
