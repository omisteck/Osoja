                ...
                <div class="row pt-2">
                    <div class="content container-fluid">
                        ...
                        <div class="page-header">
                            <h1 class="page-header-title">{{ translate('messages.conversation') }}
                                {{ translate('messages.list') }}</h1>
                        </div>
                        <!-- End Page Header -->
                        ...
                        <div class="row g-3">
                                ...
                                <div class="card">
                                    <div class="card-header border-0">
                                        <div class="input-group input---group">
                                            ...
                                            <input type="text" class="form-control border-inline-start-0 pl-1"
                                                id="serach" placeholder="Search" aria-label="Username"
                                                aria-describedby="basic-addon1" autocomplete="off">
                                        </div>
                                    </div>
                                    <input type="hidden" id="vendor_id" value="{{ $store->id }}">
                                    <!-- Body -->
                                    <div class="card-body p-0" style="overflow-y: scroll;height: 600px"
                                        id="vendor-conversation-list">
                            ...
                            <div class="col-lg-8 col-nd-6" id="vendor-view-conversation">
                                <center style="margin-top: 10%">
                                    <h4 style="color: rgba(113,120,133,0.62)">{{ translate('messages.view') }}
                                        {{ translate('messages.conversation') }}
                                    </h4>
                                </center>
                                {{-- view here --}}
                            </div>
...
@push('script_2')
    <script>
        function viewConvs(url, id_to_active, conv_id, sender_id) {
            ...
            let new_url = "{{ route('admin.store.view', ['store' => $store->id, 'tab' => 'conversations']) }}" +
                '?conversation=' + conv_id + '&user=' + sender_id;
            $.get({
                url: url,
        ...
        $('#vendor-conversation-list').scroll(function() {
            if ($('#vendor-conversation-list').scrollTop() + $('#vendor-conversation-list').height() >= $(
