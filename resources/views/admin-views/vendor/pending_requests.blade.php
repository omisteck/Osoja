...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title"><i class="tio-filter-list"></i> {{ translate('messages.new_joining_requests') }}
            </h1>
                    ...
                    <div class="select-item">
                        <select name="zone_id" class="form-control js-select2-custom"
                            ...
                            <option value="" {{ !request('zone_id') ? 'selected' : '' }}>
                                {{ translate('messages.All Zones') }}</option>
                ...
                <div class="col-md-12">
                    <div class="js-nav-scroller hs-nav-scroller-horizontal mt-2">
                        ...
                        <ul class="nav nav-tabs mb-3 border-0 nav--tabs">
                            <li class="nav-item">
                                <a class="nav-link active" href="{{ route('admin.store.pending-requests') }}"
                                    aria-disabled="true">{{ translate('messages.pending_stores') }}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('admin.store.deny-requests') }}"
                                    aria-disabled="true">{{ translate('messages.denied_stores') }}</a>
                            </li>
                        </ul>
                        <!-- End Nav -->
                    </div>
        ...
        <div class="card">
            ...
            <div class="card-header py-2">
                <div class="search--button-wrapper">
                    <h5 class="card-title">{{ translate('messages.stores') }} {{ translate('messages.list') }} <span
                            class="badge badge-soft-dark ml-2" id="itemCount">{{ $stores->total() }}</span></h5>
                    <form action="javascript:" id="search-form" class="search-form">
                        ...
                        <div class="input-group input--group">
                            <input id="datatableSearch_" type="search" id="search" name="search" class="form-control"
                                ...
                                value="{{ isset($search_by) ? $search_by : '' }}"aria-label="{{ translate('messages.search') }}"
                                required>
                                    ...
                                    @if (isset($store->vendor->status))
                                        @if ($store->vendor->status)
                                            <label class="toggle-switch toggle-switch-sm"
                                                ...
                                                <input type="checkbox"
                                                    onclick="status_change_alert('{{ route('admin.store.status', [$store->id, $store->status ? 0 : 1]) }}', '{{ translate('messages.you_want_to_change_this_store_status') }}', event)"
                                ...
                                <td>
                                    <div class="btn--container">
                                        @if ($store->vendor->status == 0)
                                            <a class="btn action-btn btn--primary btn-outline-primary float-right mr-2"
                                        ...
                                        @endif
                                        @if (!isset($store->vendor->status))
                                            <a class="btn action-btn btn--danger btn-outline-danger float-right"
                                                data-toggle="tooltip" data-placement="top"
