                ...
                <div class="row align-items-center py-2">
                    <div class="col-sm mb-2 mb-sm-0">
                        <div class="d-flex align-items-center">
                            <img onerror="this.src='{{ asset('/assets/admin/img/parcel.svg') }}'"
                                src="{{ asset('storage/module') }}/{{ $mod->icon }}" width="38" alt="img">
                            <div class="w-0 flex-grow pl-2">
                                <h1 class="page-header-title mb-0">{{ $mod->module_name }}
                                    {{ translate('messages.Dashboard') }}.</h1>
                                <p class="page-header-text m-0">{{ translate('Hello, Here You Can Manage Your') }}
                                    {{ $mod->module_name }} {{ translate('orders by Zone.') }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body pt-0">
                    <div class="d-flex flex-wrap align-items-center justify-content-between statistics--title-area">
                        ...
                        <div class="statistics--select">
                            <select class="custom-select border-0" name="statistics_type"
                    ...
                    <div class="row g-2" id="order_stats">
                        <div class="col-sm-6 col-lg-3">
                            <!-- Card -->
                            <a class="resturant-card dashboard--card __dashboard-card card--bg-1"
                                href="{{ route('admin.parcel.list', ['searching_for_deliverymen']) }}">
                                <span class="meter">
                                    <span
                                        style="height:{{ $data['total_orders'] > 0 ? ($data['searching_for_dm'] * 100) / $data['total_orders'] : 0 }}%"></span>
                                </span>
                                <h4 class="title">{{ $data['searching_for_dm'] }}</h4>
                                <span class="subtitle">{{ translate('unassigned_orders') }}</span>
                                <img src="{{ asset('/assets/admin/img/dashboard/1.png') }}" alt="img"
                        ...
                        <div class="col-sm-6 col-lg-3">
                            <!-- Card -->
                            <a class="resturant-card dashboard--card __dashboard-card card--bg-2"
                                href="{{ route('admin.parcel.orders', ['item_on_the_way']) }}">
                                <span class="meter">
                                    <span
                                        style="height:{{ $data['total_orders'] > 0 ? ($data['picked_up'] * 100) / $data['total_orders'] : 0 }}%"></span>
                                </span>
                                <h4 class="title">{{ $data['picked_up'] }}</h4>
                                <span class="subtitle">{{ translate('out_for_delivery') }}</span>
                                <img src="{{ asset('/assets/admin/img/dashboard/4.png') }}" alt="img"
                                    class="resturant-icon">
                            </a>
                            <!-- End Card -->
                        </div>
                        <div class="col-sm-6 col-lg-3">
                            ...
                            <a class="resturant-card dashboard--card __dashboard-card card--bg-3"
                                href="{{ route('admin.parcel.orders', ['delivered']) }}">
                                <span class="meter">
                                    <span
                                        style="height:{{ $data['total_orders'] > 0 ? ($data['delivered'] * 100) / $data['total_orders'] : 0 }}%"></span>
                                </span>
                                <h4 class="title">{{ $data['delivered'] }}</h4>
                                <span class="subtitle">{{ translate('delivered') }}</span>
                                <img src="{{ asset('/assets/admin/img/dashboard/2.png') }}" alt="img"
                                    class="resturant-icon">
                            </a>
                            <!-- End Card -->
                        </div>
                        ...
                        <div class="col-sm-6 col-lg-3">
                            ...
                            <a class="resturant-card dashboard--card __dashboard-card card--bg-4"
                                href="{{ route('admin.parcel.orders', ['failed']) }}">
                                <span class="meter">
                                    <span
                                        style="height:{{ $data['total_orders'] > 0 ? ($data['refund_requested'] * 100) / $data['total_orders'] : 0 }}%"></span>
                                </span>
                                <h4 class="title">{{ $data['refund_requested'] }}</h4>
                                <span class="subtitle">{{ translate('Failed Orders') }}</span>
                                <img src="{{ asset('/assets/admin/img/dashboard/5.png') }}" alt="img"
                                    class="resturant-icon">
                            </a>
                            <!-- End Card -->
                        </div>
                            ...
                            <div class="d-flex flex-wrap justify-content-between align-items-center __gap-12px">
                                ...
                                <select class="custom-select border-0 text-center w-auto ml-auto" name="commission_overview"
                                    ...
                                    <option value="this_year"
                                        {{ $params['commission_overview'] == 'this_year' ? 'selected' : '' }}>
                                        {{ translate('This year') }}
                                    </option>
                                    <option value="this_month"
                                        {{ $params['commission_overview'] == 'this_month' ? 'selected' : '' }}>
                                        {{ translate('This month') }}
                                    </option>
                                    <option value="this_week"
                                        {{ $params['commission_overview'] == 'this_week' ? 'selected' : '' }}>
                                        {{ translate('This week') }}
                                    </option>
                                </select>
                            </div>
                    ...
                    <div class="card h-100">
                        ...
                        <div class="card-header border-0">
                            ...
                            <select class="custom-select border-0 text-center w-auto" name="user_overview"
                                ...
                                <option value="this_year" {{ $params['user_overview'] == 'this_year' ? 'selected' : '' }}>
                                    {{ translate('This year') }}
                                </option>
                                <option value="this_month"
                                ...
                                <option value="this_week" {{ $params['user_overview'] == 'this_week' ? 'selected' : '' }}>
                                    {{ translate('This week') }}
                                </option>
                            ...
                            <div class="position-relative pie-chart">
                                ...
                                <div class="total--orders">
                                    <h3 class="text-uppercase mb-xxl-2">
                                        {{ $data['customer'] + $data['stores'] + $data['delivery_man'] }}</h3>
...
@push('script_2')
    ...
    <script>
        var options = {
            series: [{
                name: 'Gross Sale',
                data: [{{ $total_sell[1] }}, {{ $total_sell[2] }}, {{ $total_sell[3] }},
                    {{ $total_sell[4] }}, {{ $total_sell[5] }}, {{ $total_sell[6] }},
                    {{ $total_sell[7] }}, {{ $total_sell[8] }}, {{ $total_sell[9] }},
                    {{ $total_sell[10] }}, {{ $total_sell[11] }}, {{ $total_sell[12] }}
                ]
            }, {
                name: 'Admin Comission',
                data: [{{ $commission[1] }}, {{ $commission[2] }}, {{ $commission[3] }},
                    {{ $commission[4] }}, {{ $commission[5] }}, {{ $commission[6] }},
                    {{ $commission[7] }}, {{ $commission[8] }}, {{ $commission[9] }},
                    {{ $commission[10] }}, {{ $commission[11] }}, {{ $commission[12] }}
                ]
            }, {
                name: 'Delivery Comission',
                data: [{{ $delivery_commission[1] }}, {{ $delivery_commission[2] }},
                    {{ $delivery_commission[3] }}, {{ $delivery_commission[4] }},
                    {{ $delivery_commission[5] }}, {{ $delivery_commission[6] }},
                    {{ $delivery_commission[7] }}, {{ $delivery_commission[8] }},
                    {{ $delivery_commission[9] }}, {{ $delivery_commission[10] }},
                    {{ $delivery_commission[11] }}, {{ $delivery_commission[12] }}
                ]
            }],
            chart: {
                height: 350,
