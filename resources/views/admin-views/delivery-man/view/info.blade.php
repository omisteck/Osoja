                            ...
                            <ul class="nav nav-tabs mb-3 border-0 nav--tabs">
                                <li class="nav-item">
                                    <a class="nav-link active"
                                        href="{{ route('admin.users.delivery-man.preview', ['id' => $dm->id, 'tab' => 'info']) }}"
                                        aria-disabled="true">{{ translate('messages.info') }}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link"
                                        href="{{ route('admin.users.delivery-man.preview', ['id' => $dm->id, 'tab' => 'transaction']) }}"
                                        aria-disabled="true">{{ translate('messages.transaction') }}</a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link"
                                        href="{{ route('admin.users.delivery-man.preview', ['id' => $dm->id, 'tab' => 'conversation']) }}"
        ...
        <div class="row mb-3 row-3">
            ...
            <div class="col-sm-6 col-md-4">
                <div class="resturant-card card--bg-2">
                    ...
                    <img class="resturant-icon w--30" src="{{ asset('assets/admin/img/withdraw-amount.png') }}"
                        alt="img">
                </div>
            </div>
                ...
                <div class="search--button-wrapper">
                    <h4 class="card-title">
                        ...
                        (@if ($dm->zone)
                            {{ $dm->zone->name }}
                        @else
                            {{ translate('messages.zone') . ' ' . translate('messages.deleted') }}
                        @endif )
                        @if ($dm->application_status == 'approved')
                            @if ($dm['status'])
                                @if ($dm['active'])
                        ...
                        @else
                            <label
                                class="badge badge-soft-{{ $dm->application_status == 'pending' ? 'info' : 'danger' }}">{{ translate('messages.' . $dm->application_status) }}</label>
                        @endif
                    </h4>
                    @if ($dm->application_status == 'approved')
                        <a href="javascript:"
                            onclick="request_alert('{{ route('admin.users.delivery-man.status', [$dm['id'], $dm->status ? 0 : 1]) }}','{{ $dm->status ? translate('messages.you_want_to_suspend_this_deliveryman') : translate('messages.you_want_to_unsuspend_this_deliveryman') }}')"
                            class="btn font-medium {{ $dm->status ? 'btn--danger' : 'btn-success' }}">
                    ...
                    <div class="hs-unfold">
                        <div class="dropdown">
                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton"
                                ...
                                ({{ $dm->earning ? translate('messages.freelancer') : translate('messages.salary_based') }})
                            </button>
                        ...
                        <div class="d-flex align-items-center justify-content-center">
                            ...
                            <div class="d-block">
                                <div class="rating--review">
                                    <h1 class="title">
                                        {{ count($dm->rating) > 0 ? number_format($dm->rating[0]->average, 1) : 0 }}<span
