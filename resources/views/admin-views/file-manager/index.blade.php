            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        @endphp
                        <h5 class="card-title">{{ end($pwd) }} <span class="badge badge-soft-dark ml-2"
                                id="itemCount">{{ count($data) }}</span></h5>
                        <a class="btn btn-sm badge-soft-primary" href="{{ url()->previous() }}"><i
                                class="tio-arrow-long-left mr-2"></i>{{ translate('messages.back') }}</a>
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                                                ...
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        <a class="btn btn-primary"
                                                            ...
                                                            {{ translate('messages.download') }} </a>
                                                        <button class="btn btn-info"
                                                            onclick="copy_test('{{ $file['db_path'] }}')"><i
                                                                class="tio-copy"></i> Copy path</button>
                                                        <form
                                                            action="{{ route('admin.file-manager.destroy', base64_encode($file['path'])) }}"
                                                            method="post" onsubmit="form_submit_warrning(event)">
                                                            @csrf
                                                            @method('delete')
        ...
        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ translate('messages.upload') }}
                            {{ translate('messages.file') }} </h5>
                        ...
                        <form action="{{ route('admin.file-manager.image-upload') }}" method="post"
                            ...
                            <div class="form-group">
                                <div class="custom-file">
                                    ...
                                    <label class="custom-file-label" id="zipFileLabel"
                                        for="customZipFileUpload">{{ translate('messages.upload_zip_file') }}</label>
                                </div>
                            </div>
                            ...
                            <div class="form-group mb-0">
                                <input class="btn btn-primary" type="submit"
                                    value="{{ translate('messages.upload') }}">
                            </div>
                        </form>
                    </div>
                </div>
            ...
            for (var i = 0; i < input.files.length; i++) {
                if (input.files && input.files[i]) {
                    ...
                    reader.onload = function(e) {
                        $('#files').append(
                            '<div class="col-md-2 col-sm-4 m-1"><img class="initial--7" id="viewer" src="' + e
                            .target.result + '"/></div>');
                    }
