            ...
            <div class="card-body">
                <div class="row align-items-md-center gx-md-5">
                    <div class="col-md-4 mb-3 mb-md-0">
                        <img class="rounded initial--5" src="{{ asset('storage/campaign') }}/{{ $campaign->image }}"
                            onerror="this.src='{{ asset('assets/admin/img/160x160/img2.jpg') }}'" alt="Image Description">
                    </div>

                    <div class="col-md-8">
                        <h4>{{ translate('messages.short') }} {{ translate('messages.description') }} : </h4>
                        <p>{{ $campaign->description }}</p>
                        <form action="{{ route('admin.campaign.addstore', $campaign->id) }}" id="store-add-form"
                            method="POST">
                            @csrf
                            <!-- Search -->
                            <div class="d-flex flex-wrap g-2">
                                <div class="flex-grow-1">
                                    <?php
                                    $allstores = App\Models\Store::Active()
                                        ->where('module_id', $campaign->module_id)
                                        ->get(); ?>
                                ...
                                <div>
                                    <button type="submit" class="btn btn--primary font-weight-regular h--45px"><i
                                            class="tio-add-circle-outlined"></i> {{ translate('messages.add') }}
                                        {{ translate('messages.store') }}</button>
                                </div>
                            </div>
                            <!-- End Search -->
                        </form>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn--danger btn-outline-danger action-btn" href="javascript:"
                                        onclick="form_alert('campaign-{{ $dm->id }}','{{ translate('messages.want_to_remove_store') }}')"
