                        ...
                        <div class="card-body">
                                    ...
                                    <div class="{{ $lang != $default_lang ? 'd-none' : '' }} lang_form"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" {{ $lang == $default_lang ? 'required' : '' }}
                                                name="title[]" id="{{ $lang }}_title" class="form-control"
                                                placeholder="{{ translate('messages.new_campaign') }}"
                                                value="{{ $translate[$lang]['title'] ?? $campaign['title'] }}"
                            ...
                            @else
                                <div id="{{ $default_lang }}-form">
                                    <div class="form-group">
                                        <label class="input-label"
                                            for="exampleFormControlInput1">{{ translate('messages.title') }} (EN)</label>
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-md-3 col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="store_id" class="js-data-example-ajax form-control" id="store_id"
                                            onchange="getStoreData('{{ url('/') }}/admin/store/get-addons?data[]=0&store_id='+this.value,'add_on')"
                                            data-toggle="tooltip" data-placement="right"
                                            data-original-title="{{ translate('messages.select_store') }}" disabled>
                                            @if ($campaign->store)
                                                <option value="{{ $campaign->store->id }}" selected>
                                                    {{ $campaign->store->name }}</option>
                                            @else
                                ...
                                <div class="col-md-3 col-sm-6" id="stock_input">
                                    <div class="form-group mb-0">
                                        ...
                                        <input type="number" class="form-control" name="current_stock"
                                            value="{{ $campaign->stock }}" id="quantity">
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6" id="addon_input">
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="addon_ids[]" id="add_on" class="form-control js-select2-custom"
                                            multiple="multiple">
                                            @foreach (\App\Models\AddOn::orderBy('name')->get() as $addon)
                                                <option value="{{ $addon['id'] }}"
                                                    {{ in_array($addon->id, json_decode($campaign['add_ons'], true)) ? 'selected' : '' }}>
                                                    {{ $addon['name'] }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-3 co-sml-6">
                                    <div class="form-group" id="veg_non_veg">
                                        ...
                                        <select name="veg" class="form-control js-select2-custom">
                                            <option value="0" {{ $campaign['veg'] == 0 ? 'selected' : '' }}>
                                                {{ translate('messages.non_veg') }}</option>
                                            <option value="1" {{ $campaign['veg'] == 1 ? 'selected' : '' }}>
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                <div class="col-md-3 col-sm-6">
                                    <div class="form-group mb-0">
                                        ...
                                        <input type="number" min="1" max="100000" step="0.01"
                                            value="{{ $campaign->price }}" name="price" class="form-control"
                                ...
                                <div class="col-md-3 col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="exampleFormControlInput1">{{ translate('messages.discount') }}<span
                                        ...
                                        <input type="number" min="0" max="100000"
                                            value="{{ $campaign->discount }}" name="discount" class="form-control"
                                    ...
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="discount_type" class="form-control js-select2-custom">
                                            <option value="percent"
                                                {{ $campaign->discount_type == 'percent' ? 'selected' : '' }}>
                                                {{ translate('messages.percent') }}</option>
                                            <option value="amount"
                                                {{ $campaign->discount_type == 'amount' ? 'selected' : '' }}>
                                                {{ translate('messages.amount') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-3 co-sml-6">
                                        ...
                                        <select name="unit" class="form-control js-select2-custom">
                                            @foreach (\App\Models\Unit::all() as $unit)
                                                <option value="{{ $unit->id }}"
                                                    {{ $unit->id == $campaign->unit_id ? 'selected' : '' }}>
                                                    {{ $unit->unit }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <select name="attribute_id[]" id="choice_attributes"
                                        ...
                                        @foreach (\App\Models\Attribute::orderBy('name')->get() as $attribute)
                                            <option value="{{ $attribute['id'] }}"
                                                {{ in_array($attribute->id, json_decode($campaign['attributes'], true)) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-12">
                                <div class="customer_choice_options" id="customer_choice_options">
                                    @include('admin-views.product.partials._choices', [
                                        'choice_no' => json_decode($campaign['attributes']),
                                        'choice_options' => json_decode($campaign['choice_options'], true),
                                    ])
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="variant_combination" id="variant_combination">
                                    @include('admin-views.product.partials._edit-combinations', [
                                        'combinations' => json_decode($campaign['variations'], true),
                                        'stock' => config('module.' . $campaign->module->module_type)['stock'],
                                    ])
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-md-3 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="date_from" class="form-control" required=""
                                        name="start_date" value="{{ $campaign->start_date->format('Y-m-d') }}">
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" id="date_to" class="form-control" required=""
                                        name="end_date" value="{{ $campaign->end_date->format('Y-m-d') }}">
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="time" id="start_time" class="form-control" name="start_time"
                                        value="{{ $campaign->start_time->format('H:i') }}">
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="time" id="end_time" class="form-control" name="end_time"
                                        value="{{ $campaign->end_time->format('H:i') }}">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function() {
        @if (count(json_decode($campaign['add_ons'], true)) > 0)
            getStoreData(
                '{{ url('/') }}/admin/store/get-addons?store_id={{ $campaign['store_id'] }}@foreach (json_decode($campaign['add_ons'], true) as $addon)&data[]={{ $addon }}@endforeach',
                'add_on');
        @else
            getStoreData(
                '{{ url('/') }}/admin/store/get-addons?data[]=0&store_id={{ $campaign['store_id'] }}',
                'add_on');
        @endif
    });
</script>
...
<script>
    ...
    function add_more_customer_choice_option(i, name) {
        ...
        $('#customer_choice_options').append(
            ...
            '" placeholder="Choice Title" readonly></div><div class="col-sm-9"><input type="text" class="form-control" name="choice_options_' +
            i +
            ...
            <div class="row g-2">
                ...
                `" checked onchange="show_min_max(` + count + `)"
                            ...
                            <label class="form-check form--check mr-2 mr-md-4">
                                ...
                                name="options[` + count + `][type]" id="type` + count + `" onchange="hide_min_max(` +
                count + `)"
                                >
                                <span class="form-check-label">
                                    {{ translate('Single') }}
