    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title mr-3 mb-md-0">
                ...
                <span>
                    {{ translate('messages.store') }} {{ translate('messages.withdraw') }}
                    {{ translate('messages.information') }}
                </span>
            </h1>
        </div>
        <!-- Page Heading -->
        ...
        <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-4">
                                <div class="d-flex __gap-5px">
                                    <span>{{ translate('messages.note') }}</span><span>:</span><span>
                                        {{ $wr->transaction_note }}</span>
                                </div>
                            </div>
                            <div class="col-4">
                                @if ($wr->approved == 0)
                    ...
                    <div class="card-body">
                        <div class="col-md-8 mt-2">
                            <h4 class="d-flex __gap-5px"><span>{{ translate('messages.bank_name') }}</span>
                                <span>:</span><span>
                                    {{ $wr->vendor && $wr->vendor->bank_name ? $wr->vendor->bank_name : 'No Data found' }}</span>
                            </h4>
                            <h6 class="text-capitalize d-flex __gap-5px"><span>{{ translate('messages.branch') }}</span>
                                <span>:</span>
                                <span>{{ $wr->vendor && $wr->vendor->branch ? $wr->vendor->branch : 'No Data found' }}</span>
                            </h6>
                            <h6 class="d-flex __gap-5px"><span>{{ translate('messages.holder_name') }}</span>
                                <span>:</span><span>
                                    {{ $wr->vendor && $wr->vendor->holder_name ? $wr->vendor->holder_name : 'No Data found' }}</span>
                            </h6>
                            <h6 class="d-flex __gap-5px"><span>{{ translate('messages.account_no') }}</span>
                                <span>:</span><span>
                                    {{ $wr->vendor && $wr->vendor->account_no ? $wr->vendor->account_no : 'No Data found' }}</span>
                            </h6>
                        </div>
                    </div>
                </div>
            ...
            @if ($wr->vendor && $wr->vendor->stores[0])
                <div class="col-md-4">
                    <div class="card min-height-260">
                        ...
                        <div class="card-body">
                            <h5 class="d-flex __gap-5px"><span>{{ translate('messages.store') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->stores[0]->name }}</span></h5>
                            <h5 class="d-flex __gap-5px"><span>{{ translate('messages.phone') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->stores[0]->contact }}</span></h5>
                            <h5 class="d-flex __gap-5px"><span>{{ translate('messages.address') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->stores[0]->address }}</span></h5>
                            <h5 class="text-capitalize badge badge-success d-flex __gap-5px">
                                <span>{{ translate('messages.balance') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->wallet->balance }}</span></h5>
                        </div>
                    </div>
                </div>
            @endif
            <div class="col-md-4">
                <div class="card min-height-260">
                    ...
                    <div class="card-body">
                        @if ($wr->vendor)
                            <h5 class="d-flex __gap-5px"><span>{{ translate('messages.name') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->f_name }} {{ $wr->vendor->l_name }}</span></h5>
                            <h5 class="d-flex __gap-5px"><span>{{ translate('messages.email') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->email }}</span></h5>
                            <h5 class="d-flex __gap-5px"><span>{{ translate('messages.phone') }}</span> <span>:</span>
                                <span>{{ $wr->vendor->phone }}</span></h5>
                        @else
                            ...
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="recipient-name"
                                        class="col-form-label">{{ translate('messages.request') }}:</label>
                                ...
                                <div class="form-group">
                                    <label for="message-text"
                                        class="col-form-label">{{ translate('Note about transaction or
                                                                            request') }}:</label>
