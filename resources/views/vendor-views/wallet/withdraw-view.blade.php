    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{ translate('messages.seller') }}
                    {{ translate('messages.Withdraw') }}</li>
            </ol>
        </nav>
        ...
        <div class="d-sm-flex row align-items-center justify-content-between mb-2">
            <div class="col-md-6">
                <h4 class=" mb-0 text-black-50">{{ translate('messages.seller') }} {{ translate('messages.Withdraw') }}
                    {{ translate('messages.information') }}</h4>
            </div>
        ...
        <div class="row mt-3">
            <div class="col-md-6">
                <div class="card">
                    ...
                    <div class="card-body">
                        <h5>{{ translate('messages.name') }} : {{ $seller->seller->f_name }}
                            {{ $seller->seller->l_name }}</h5>
            ...
            <div class="col-md-6">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3 class="h3 mb-0  ">{{ translate('messages.Withdraw') }} {{ translate('messages.information') }}
                        </h3>
                    </div>
                    <div class="card-body">
                        ...
                        @if ($seller->approved == 0)
                            ...
                            <div class="text-center mt-3">
                                <form class="d-inline-block" action="{{ route('admin.sellers.withdraw_status') }}"
                                    ...
                                    <button type="submit"
                                        class="btn btn-primary">{{ translate('messages.Approve') }}</button>
                                </form>
                                <form class="d-inline-block" action="{{ route('admin.sellers.withdraw_status') }}"
                                    method="POST">
                                    @csrf
                                    ...
                                    <button type="submit"
                                        class="btn btn-danger">{{ translate('messages.Denied') }}</button>
                                </form>
                            </div>
                        @else
                            <div class="text-center col-sm-3  mt-3">
                                ...
                                @if ($seller->approved == 1)
                                    <label
                                        class="badge badge-success p-2 rounded-bottom">{{ translate('messages.Approved') }}</label>
                                @else
                                    <label
                                        class="badge badge-danger p-2 rounded-bottom">{{ translate('messages.Denied') }}</label>
                                @endif
