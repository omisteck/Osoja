    ...
    <div class="row g-2">
        @foreach ($most_rated_items as $key => $item)
            <div class="col-md-4 col-6">
                <div class="grid-card top--rated-food pb-4 cursor-pointer"
                    onclick="location.href='{{ route('vendor.item.view', [$item['id']]) }}'">
                    <center>
                        <img class="rounded" src="{{ asset('storage/product') }}/{{ $item['image'] }}"
                    ...
                    <div class="text-center mt-3">
                        <h5 class="name m-0 mb-1">
                            {{ Str::limit($item->name ?? translate('messages.Item deleted!'), 20, '...') }}</h5>
                        <div class="rating">
