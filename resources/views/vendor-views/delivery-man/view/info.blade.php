                ...
                <ul class="nav nav-tabs mb-3 border-0 nav--tabs">
                    <li class="nav-item">
                        <a class="nav-link active"
                            href="{{ route('vendor.delivery-man.preview', ['id' => $dm->id, 'tab' => 'info']) }}"
                            aria-disabled="true">{{ translate('messages.info') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link"
                            href="{{ route('vendor.delivery-man.preview', ['id' => $dm->id, 'tab' => 'transaction']) }}"
        ...
        <div class="row mb-3 g-3 justify-content-center">
            ...
            <div class="col-sm-6 col-md-4">
                <div class="resturant-card card--bg-1">
                    ...
                    <h5 class="subtitle">
                        {{ translate('messages.total') }} {{ translate('messages.delivered') }}
        ...
        <div class="card mb-3 mb-lg-5">
            <div class="card-header py-2">
                <div class="search--button-wrapper">
                    <h4 class="card-title mb-md-0">{{ $dm['f_name'] . ' ' . $dm['l_name'] }}@if ($dm['status'])
                            @if ($dm['active'])
                                <label class="badge badge-soft-primary m-0 ml-2">{{ translate('messages.online') }}</label>
                            @else
                                <label class="badge badge-soft-danger m-0 ml-2">{{ translate('messages.offline') }}</label>
                            @endif
                        @else
                    ...
                    <a href="javascript:"
                        onclick="request_alert('{{ route('vendor.delivery-man.status', [$dm['id'], $dm->status ? 0 : 1]) }}','{{ $dm->status ? 'Want to suspend this deliveryman ?' : 'Want to unsuspend this deliveryman' }}')"
                        class="btn {{ $dm->status ? 'btn-danger' : 'btn-success' }}">
